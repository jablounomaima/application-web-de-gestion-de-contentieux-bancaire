...
@Component({
    ...
    <div class="page-container">
      ...
      <div *ngFor="let dossier of dossiersFiltres()" class="dossier-block">
        ...
        <div class="dossier-header"
          ...
          <div class="dossier-right">
            ...
            <div class="total-badge">
              <div class="total-ht">
                HT : {{ dossier.totalHT | number:'1.3-3' }} TND
              </div>
              <div class="total-ttc">
                TTC : {{ dossier.totalTTC | number:'1.3-3' }} TND
              </div>
            </div>
            <span class="nb-factures">
              {{ dossier.factures.length }} facture(s)
            </span>
            <span class="toggle-icon">
              {{ isDossierOpen(dossier.dossierId) ? '▲' : '▼' }}
            </span>
          </div>
        </div>
        ...
        <div class="factures-table-wrap"
             *ngIf="isDossierOpen(dossier.dossierId)">
          <table class="factures-table">
              ...
              <tr *ngFor="let f of dossier.factures"
                ...
                <td>
                  <div class="prestataire-cell">
                    <div class="prestataire-avatar"
                         [ngClass]="avatarClass(f.prestataireType)">
                ...
                <td>
                  <span class="type-badge"
                        [ngClass]="typeBadgeClass(f.prestataireType)">
                ...
                <!-- TVA -->
                <td class="tva-col">
                  {{ f.montantHT * 0.19 | number:'1.3-3' }} TND
                </td>

                <!-- TTC -->
                <td class="ttc-val">
                  {{ f.montantTTC | number:'1.3-3' }} TND
                </td>

                <!-- Statut -->
                <td>
                  <span class="statut-pill"
                        [ngClass]="statutClass(f)">
                    {{ statutLabel(f) }}
                  </span>
                  <div *ngIf="isRejete(f) && f.commentaire"
                       class="rejet-motif">
                    💬 {{ f.commentaire }}
                  </div>
                  <div *ngIf="f.dateFacture && isValidee(f)"
                ...
                <td>
                  <div class="action-btns" *ngIf="peutEtreTraitee(f)">
                    <button class="btn-valider"
                            (click)="validerFacture(f, true, dossier)"
                            [disabled]="f.saving">
                      <span *ngIf="!f.saving">✓ Valider</span>
                      <span *ngIf="f.saving">...</span>
                    </button>
                    <button class="btn-rejeter"
                            (click)="ouvrirModal(f, dossier)"
                            [disabled]="f.saving">
                      ✕ Rejeter
                    </button>
                  </div>
                  <div *ngIf="isValidee(f)" class="traite-label valide-label">
                    ✅ Validée
                  </div>
                  <div *ngIf="isRejete(f)" class="traite-label rejete-label">
                    ❌ Rejetée
                  </div>
                </td>
            ...
            <tfoot>
              <tr class="total-row">
                ...
                <td>
                  <strong>{{ dossier.totalHT | number:'1.3-3' }} TND</strong>
                </td>
                <td>
                  <strong>
                    {{ dossier.totalHT * 0.19 | number:'1.3-3' }} TND
                  </strong>
                </td>
                <td class="ttc-val">
                  <strong>{{ dossier.totalTTC | number:'1.3-3' }} TND</strong>
                </td>
                <td colspan="2"></td>
              </tr>
            </tfoot>
    ...
    <div class="modal-overlay" *ngIf="modalVisible"
      ...
      <div class="modal-box" (click)="$event.stopPropagation()">
        ...
        <textarea [(ngModel)]="commentaireRejet"
                  ...
                  placeholder="Ex: Montant incorrect, référence manquante...">
        </textarea>
  ...
  styles: [`
    * { box-sizing: border-box; }
    ...
    .btn-rejeter:disabled { opacity: 0.5; cursor: not-allowed; }
...
export class ValidateurFinancierFacturesComponent implements OnInit {
  ...
  charger(): void {
    ...
    this.http.get<any>(`${this.api}/factures`).subscribe({
      next: (res) => {
        ...
        // ✅ Ajouter ce log
        console.log('FACTURES:', JSON.stringify(this.dossiers[0]?.factures, null, 2));
  ...
  toggleDossier(id: number): void {
    if (this.dossiersOuverts.has(id)) {
  ...
  validerFacture(facture: any, valide: boolean, dossier: any): void {
    ...
    const body = {
      valide,
      commentaire: valide ? '' : this.commentaireRejet
    };

    this.http.post(
      `${this.api}/missions/${facture.missionId}/valider-facture`,
      body
    ).subscribe({
      next: (res: any) => {
        facture.saving        = false;
        facture.factureValide = valide;
        facture.statutMission = valide
                                ? 'FACTURE_VALIDEE'
                                : 'FACTURE_REJETEE';
        if (!valide) facture.commentaire = this.commentaireRejet;
        this.showToast(
          valide ? '✓ Facture validée avec succès !'
                 : '✓ Facture rejetée.',
          'success'
        );
        this.fermerModal();
      ...
      error: (e) => {
        facture.saving = false;
        this.showToast(
          e.error?.error || 'Erreur lors de la validation.',
          'error'
        );
      }
    });
  ...
  isValidee(f: any): boolean {
    ...
    // ✅ Vérifier statutMission EN PRIORITÉ
    if (f.statutMission && statutsValides.includes(f.statutMission)) return true;
    // ✅ Sinon vérifier factureValide — seulement si explicitement true
  ...
  isRejete(f: any): boolean {
    ...
    // ✅ Seulement si explicitement false ET qu'il y a un commentaire
    return f.factureValide === false && !!f.commentaire;
  }

  peutEtreTraitee(f: any): boolean {
    // ✅ En attente si pas encore validée ni rejetée
    if (this.isValidee(f)) return false;
    if (this.isRejete(f))  return false;
    // ✅ Statuts qui indiquent une facture soumise en attente
  ...
  avatarClass(type: string): string {
    const map: Record<string, string> = {
      'AVOCAT':   'avatar-avocat',
      'EXPERT':   'avatar-expert',
      'HUISSIER': 'avatar-huissier'
    };
