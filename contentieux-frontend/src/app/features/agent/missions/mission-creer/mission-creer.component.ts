...
export class MissionCreerComponent implements OnInit {
  ...
  // ── Mapping centralisé ────────────────────────────────────────
  private readonly PRESTATION_MAP: Record<string, { label: string; typePrestataire?: string }> = {
    PROCEDURE_JUDICIAIRE: { label: '⚖️ Procédure judiciaire', typePrestataire: 'AVOCAT'   },
    EXPERTISE:            { label: '🔬 Expertise',            typePrestataire: 'EXPERT'   },
    SIGNIFICATION:        { label: '📜 Signification',        typePrestataire: 'HUISSIER' },
    RECOUVREMENT:         { label: '💰 Recouvrement'                                      },
    ENQUETE:              { label: '🔍 Enquête'                                           },
  };

  // ── Types de prestation disponibles (depuis l'enum backend) ───
  readonly typesPrestation = [
    { value: 'PROCEDURE_JUDICIAIRE', label: '⚖️ Procédure judiciaire' },
  ...
  // ── Formulaire ─────────────────────────────────────────────────
  form = {
    typePrestation:  '' as string,       // ← type choisi par l'agent
  ...
  soumettre(): void {
    ...
    const payload = {
      typePrestation:  this.form.typePrestation,
  ...
  private valider(): boolean {
    if (!this.form.typePrestation)     { this.formError = 'Veuillez sélectionner un type de prestation.'; return false; }
  ...
  selectionnerTypePrestation(type: string): void {
    this.form.typePrestation = type;
    this.form.prestataireId  = null;

    const typeAttendu = this.PRESTATION_MAP[type]?.typePrestataire;
  ...
  getLabelTypePrestation(type: string): string {
    return this.PRESTATION_MAP[type]?.label ?? type;
  }
