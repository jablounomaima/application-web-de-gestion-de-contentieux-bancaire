...
public class DossierMissionController {
    ...
    public ResponseEntity<?> listeMissions(@PathVariable Long dossierId,
                                           Authentication auth) {
        try {
            DossierContentieux dossier = dossierRepository
                    .findByIdWithDetails(dossierId).orElse(null);
            if (dossier == null)
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Dossier introuvable"));
            ...
            for (Mission m : missions) {
                ...
                if (m.getPrestataire() != null) {
                    ...
                    pm.put("type",     m.getPrestataire().getType() != null
                                       ? m.getPrestataire().getType().name() : "");
                    mm.put("prestataire", pm);
                }
                if (m.getPrestation() != null) {
                    mm.put("typePrestation", m.getPrestation().getType() != null
                                             ? m.getPrestation().getType().name() : null);
                }
                missionsMap.add(mm);
            }
    ...
    public ResponseEntity<?> creerMission(@PathVariable Long dossierId,
        ...
        try {
            ...
            LocalDate      dateFinPrevue = (dateFinStr != null && !dateFinStr.isBlank())
                                           ? LocalDate.parse(dateFinStr.trim()) : null;
            ...
            // ── 6. Prestation (récupérer ou créer) ────────────────────
            Prestation prestation = prestationRepository.findByDossier_Id(dossierId)
                    ...
                    .orElseGet(() -> {
                        ...
                        Prestation p = Prestation.builder()
                                ...
                                .agentCreateur(agent)
                                .numeroPrestation(String.format("PREST-%d-%05d",
                                        LocalDate.now().getYear(), nb))
                                .build();
            ...
            mission.setNumeroMission(String.format("MISS-%d-%05d",
            ...
            log.error("Erreur creerMission dossierId={} : {}", dossierId, e.getMessage(), e);
